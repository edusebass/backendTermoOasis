...
const crearCita = async (req, res) => {
  ...
  if (!idPaciente || !idDoctor || !start || !end || comentarios === undefined) {
    return res.status(400).json({
      msg: "Lo sentimos, debes poner todos los campos en el body",
      status: false,
    });
  }
  ...
  if (comentarios.length > 50) {
    return res.status(400).json({
      msg: "El comentario no debe exceder los 30 caracteres",
      status: false,
    });
  }

  if (
    !mongoose.Types.ObjectId.isValid(idPaciente) ||
    !mongoose.Types.ObjectId.isValid(idDoctor)
  ) {
    return res.status(400).json({
      msg: "El ID no es válido no pertenece a mongoDB",
      status: false,
    });
  }
  ...
  try {
    ...
    if (!existePaciente) {
      return res
        .status(400)
        .json({ msg: "Paciente no se encuentra registrado", status: false });
    }
    if (!existeDoctor) {
      return res
        .status(400)
        .json({ msg: "Doctor no se encuentra registrado", status: false });
    }
    ...
    if (inicioInput < new Date() || finInput < new Date()) {
      return res
        .status(400)
        .json({ msg: "No puedes agendar citas en el pasado", status: false });
    }

    if (inicioInput.getTime() === finInput.getTime()) {
      return res.status(400).json({
        msg: "Debes elegir un horario de inicio y fin diferente",
        status: false,
      });
    }

    const diferenciaHoras =
      (finInput.getTime() - inicioInput.getTime()) / (1000 * 60 * 60);
    if (diferenciaHoras < 1) {
      return res.status(400).json({
        msg: "La diferencia entre la hora de inicio y fin debe ser de al menos una hora",
        status: false,
      });
    }
    ...
    if (diferenciaHoras !== 1) {
      return res.status(400).json({
        msg: "La cita debe durar exactamente una hora",
        status: false,
      });
    }
    ...
    if (startDateStr.substr(0, 10) !== endDateStr.substr(0, 10)) {
      return res.status(400).json({
        msg: "El inicio y fin de la cita deben ser en el mismo día",
        status: false,
      });
    }
    ...
    if (
      new Date(startDateStr).getFullYear() !== currentYear ||
      new Date(endDateStr).getFullYear() !== currentYear
    ) {
      return res.status(400).json({
        msg: "El inicio y fin de la cita deben ser en el mismo año actual",
        status: false,
      });
    }
    ...
    const existingCita = await CitaModelo.findOne({
      $or: [
        {
          start: {
            $lt: new Date(new Date(endDateStr).getTime() + 60000).toISOString(),
          }, // Permitir un minuto después de end
          end: {
            $gt: new Date(
              new Date(startDateStr).getTime() - 60000
            ).toISOString(),
          }, // Permitir un minuto antes de start
        },
        {
      ...
      isCancelado: false, // Solo considera citas que no estén canceladas
    });
...
const editarCita = async (req, res) => {
  ...
  try {
    ...
    if (!startISO || !endISO || comentarios === undefined) {
      return res
        .status(400)
        .json({ msg: "Lo sentimos, faltan campos", status: false });
    }
    ...
    if (inicioInput.getTime() === finInput.getTime()) {
      const error = new Error(
        "Debes elegir un horario de inicio y fin diferente"
      );
      return res.status(400).json({ msg: error.message, status: false });
    }
    ...
    if (
      inicioInput.toISOString().substr(0, 10) !==
      finInput.toISOString().substr(0, 10)
    ) {
      const error = new Error(
        "El inicio y fin de la cita deben ser en el mismo día"
      );
      return res.status(400).json({ msg: error.message, status: false });
    }
    ...
    if (
      inicioInput.getFullYear() !== new Date().getFullYear() ||
      finInput.getFullYear() !== new Date().getFullYear()
    ) {
      const error = new Error(
        "El inicio y fin de la cita deben ser en el mismo año actual"
      );
      return res.status(400).json({ msg: error.message, status: false });
    }
    ...
    res.status(200).json({
      response: "Cita actualizada exitosamente",
      msg: citastored,
      status: true,
    });
  } catch {
...
const cancelarCita = async (req, res) => {
  ...
  try {
    ...
    if (diferenciaTiempo < 86400000) {
      return res.status(400).json({
        msg: "No puedes cancelar la cita con menos de 24 horas de antelación",
      });
    }
